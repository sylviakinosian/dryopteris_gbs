...
sed 's/^>centroid=centroid=/>centroid=centroid=l/g' RCCludo.fasta > lRCCludo.fasta
```
...
```{bash eval=FALSE}
# cluster at 92% similarity
vsearch --cluster_fast diploid_consensus.fasta --threads 10 --iddef 2 --id 0.92 --consout 92diploidCons.fasta --msaout 92diploidMsa.fasta

# remove all clusters that are present in only one species
./presence_filter.pl 92diploidCon.fasta

finished, retained  contigs
```

```{bash eval=FALSE}
# cluster at 88% similarity
vsearch --cluster_fast diploid_consensus.fasta --threads 10 --iddef 2 --id 0.88 --consout 88diploidCons.fasta --msaout 88diploidMsa.fasta

# remove all clusters that are present in only one species
./presence_filter.pl 88diploidCons.fasta
...
# remove all clusters that are present in only one species
./presence_filter.pl 84diploidCons.fasta
...
```{bash eval=FALSE}
./entropy -b 10000 -t 4 -k 2 -i ae_in.gl -o out.hdf5 -m 1 -w 0 -q pop_ests.txt -s 20
```
