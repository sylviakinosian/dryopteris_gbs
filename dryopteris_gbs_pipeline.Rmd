...
```{bash eval=FALSE}
cat RCCgoldiana.fasta RCCintermedia.fasta RCCexpansa.fasta RCCludoviciana.fasta > diploid_consensus.fasta
```
...
REF='/path/to/diploid_consensus_final.fasta'
...
```{bash eval=FALSE}
vcftools --remove-filtered-all --remove-indels --maf 0.1 --max-maf 0.99 --min-meanDP 4.0 --max-missing 0.3 --minQ 20 --recode-INFO-all --recode --vcf all_dryopteris_variants.vcf
```
...
k2_1 <- read.csv("k1.txt", sep = ',', header = F)
k2_2 <- read.csv("k3.txt", sep = ',', header = F)
k2_3 <- read.csv("k4.txt", sep = ',', header = F)
...
make_df <- function(k_file, k_val, n_inds = 16, names){
	x <- 1
...
df2 <- make_df(k_file = avg2, k_val = 2, names = names)
