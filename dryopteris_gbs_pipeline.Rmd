...
```{bash eval=FALSE}
perl vcf2gl.pl ice_filteredVar.vcf
...
```{bash eval=FALSE}
perl gl2genest.pl ice_rawVar.gl
...
d <- read.table("pntest_out.*.txt", header = F)
...
```{bash eval=FALSE}
./estpost_entropy -o k2.txt -p q -s 0 -w 0 entropy_ae_k2_2.hdf5
```
-o outfile
...
a2 <- avg_k(kval = 2, chain1 = k2, chain2 = k2_2)
df2 <- make_df(kfile = a2, kval = 2, names = names)
a3 <- avg_k(kval = 3, chain1 = k3, chain2 = k3_2)
df3 <- make_df(kfile = a3, kval = 3, names = names)
a4 <- avg_k(kval = 4, chain1 = k4, chain2 = k4_2)
df4 <- make_df(kfile = a4, kval = 4, names = names)
a5 <- avg_k(kval = 5, chain1 = k5, chain5 = k5_2)
df5 <- make_df(kfile = a5, kval = 5, names = ei_names)
